...
class UserDetail extends Component {
    ...
    constructor(props) {
      ...
      getUserProfile() {
          fetch('http://44.240.53.177/api/pub/users/'+this.props.route.params.userId,
            {
              method: 'GET',
              headers: {
            ...
            .then( (response) =>{
              ...
              response.experiments.forEach(element => {
                console.log(element);
                var experiment = {};
                experiment.time = element.start.split('T')[0] +" - "+element.end.split('T')[0];
              ...
              response.educations.forEach(element => {
                console.log(element);
                var education = {};
                education.time = element.entering.split('T')[0] +" - "+element.departure.split('T')[0];
    ...
    getUserRate() {
      fetch('http://44.240.53.177/api/pub/rates/'+this.props.route.params.userId,
        {
          method: 'GET',
          headers: {
            'Authorization': ""+this.props.currentUser.accessToken+""
          ...
          response.forEach((item) => {
              if (item) {
                  ...
                  if (item.type === "TECHNICAL") {
                      technicalRateAvg = item.avg;
                  } else if (item.type === "TIME") {
                      timeRateAvg = item.avg;
                  } else if (item.type === "COMMUNICATION") {
                      communicationRateAvg = item.avg;
                  }
              }
          });
    ...
    return (
      <ScrollView contentContainerStyle={styles.container}>
          <View style={styles.header}>
            <View style={styles.headerContent}>
                <Image style={styles.avatar}
                  source={(this.state.userProfile.profile  &&  this.state.userProfile.profile.image!==null)?
              { uri: 'http://44.240.53.177/files/' + this.state.userProfile.profile.image } :
              { uri: 'https://bootdey.com/img/Content/avatar/avatar6.png' }}
               key={this.state.userProfile.user.id}
              />

                <Text style={styles.name}>
                  {this.state.userProfile.user.firstName} {this.state.userProfile.user.lastName}
                </Text>
                <Text style={styles.userInfo}>{this.state.userProfile.user.email}</Text>
                {this.state.userProfile.profile.city&& this.state.userProfile.profile.country&&
                  <Text style={styles.userInfo}>
                  {this.state.userProfile.profile.city.name} , {this.state.userProfile.profile.country.name}
                  </Text>}
            </View>
          </View>
          ...
          <View style={styles.cardContainer}>
            <Card >
                <View>
                 <Text style={{textAlign:'center',marginBottom:10}}>About</Text>
                </View>
                <Card.Divider/>
            ...
            <View style={styles.cardContainer}> 
                <Card  titleStyle={{textAlign: 'left'}} title={
                    ...
                    <View style={{ flex: 1, flexDirection: 'row', marginTop: 20, justifyContent: 'space-around',marginBottom:10, }}>

                      <Text style={{ marginLeft: -25 ,fontWeight:'bold'}}>Total Review </Text>
                      <Text style={{fontWeight:'bold'}} >({this.state.userProfile.totalReview})</Text>
                    </View>
                    ...
                    <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-around' }}>
                      ...
                      <AirbnbRating
                        count={5}
                        defaultRating={this.state.userProfile.technicalRateAvg}
                    ...
                    <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-around' }}>
                      ...
                      <AirbnbRating
                        count={5}
                        defaultRating={this.state.userProfile.timeRateAvg}
                        showRating={false}
                    ...
                    <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-around' }}>
                      ...
                      <AirbnbRating
                        count={5}
                        defaultRating={this.state.userProfile.communicationRateAvg}
            ...
            <Card >
                <View>
                 <Text style={{textAlign:'center',marginBottom:10}}>My Experiences</Text>
                </View>
                <Card.Divider/>
                ...
                <Timeline
                      ...
                      timeStyle={{textAlign: 'center', backgroundColor:'#ff9797', color:'white', padding:0, borderRadius:0,width:0}}
            ...
            <Card >
                <View>
                 <Text style={{textAlign:'center',marginBottom:10}}>My Education</Text>
                </View>
                <Card.Divider/>
            ...
            <Card >
                <View>
                 <Text style={{textAlign:'center',marginBottom:10}}>Skills</Text>
                </View>
                <Card.Divider/>
            ...
            <Card >
                <View>
                 <Text style={{textAlign:'center',marginBottom:10}}>Hourly Rate</Text>
                </View>
                <Card.Divider/>
...

                {
                  ...
                  <View style={{flex:5,flexDirection:'row'}}>
                     <Text style={{flex:3,textAlign:'left'}}>
                       {this.state.userProfile.hourRates[0].amount} {this.state.userProfile.hourRates[0].currency}
                     </Text>
            ...
            <Card >
                <View>
                 <Text style={{textAlign:'center',marginBottom:10}}>Contact Info</Text>
                </View>
                <Card.Divider/>

                { this.state.userProfile.user.mobile&&
                  <View style={{flex:5,flexDirection:'row',justifyContent:'center',alignItems:'center',marginTop:10}}>
                     <Icon name='phone'
                      type='font-awesome'
                      color={Color.primaryBackground}
                      style={{flex:1,textAlign:'center'}}/>
                      <Text style={{flex:4,textAlign:'left',marginLeft:10}}>
                        {this.state.userProfile.user.mobile}
                     </Text>
                </View>
                }
                ...
                { this.state.userProfile.profile.linkedin&&
                  <View style={{flex:5,flexDirection:'row',justifyContent:'center',alignItems:'center',marginTop:10}}>
                     <Icon name='linkedin'
                      ...
                      style={{flex:1,textAlign:'center'}}/>
                      <Text 
                      style={{flex:4,textAlign:'left',marginLeft:10,color: '#0645AD'}}
                      onPress={() => Linking.openURL(this.state.userProfile.profile.linkedin)}
                      >
                        {this.state.userProfile.profile.linkedin}
                     </Text>
                </View>
                ...
                { this.state.userProfile.profile.webSite&&
                  <View style={{flex:5,flexDirection:'row',justifyContent:'center',alignItems:'center',marginTop:10}}>
                     <Icon name='globe'
                      ...
                      <Text 
                      style={{flex:4,textAlign:'left',marginLeft:10,color: '#0645AD'}}
                      onPress={() => Linking.openURL(this.state.userProfile.profile.webSite)}
                      >
                        {this.state.userProfile.profile.webSite}
                     </Text>
                </View>
